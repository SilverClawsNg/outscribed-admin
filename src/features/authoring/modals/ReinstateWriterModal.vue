...
import type { ConfirmRequest } from '../types/AuthoringTypes'
...
onBeforeMount(() => {
  ...
  if (!writerStore.writer) {
    ...
    setError({
      ...
      detail: "Unable to load current writer details. Refresh page and try again",
 ...
 // --- INITIALIZE FORM DATA FROM STORE ---
  formData.value.writerId = writerStore.writer.writerId
  resetProgress()
...
// 1. Tell the ecosystem the user has initiated an action
 ...
 const { success, error } = await writerStore.reinstateWriter(formData.value!)
...
<template>
    ...
    <h2>Confirm to reinstate writer</h2>
