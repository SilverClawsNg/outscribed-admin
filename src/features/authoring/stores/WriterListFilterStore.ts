...
import { getValidGeneralSortType, getValidDateScope, 
  getValidWriterStatus, getValidCountry
...
if (queryParameters && Object.keys(queryParameters).length > 0) {
  ...
  if(queryParameters.status){
    const validatedStatus = getValidWriterStatus(queryParameters.status)
...
function getAsDictionary(): Record<string, string> {
  ...
  return {
    sort, startdate, enddate, scope, keyword, pointer, username, bypasscache, country, status,
