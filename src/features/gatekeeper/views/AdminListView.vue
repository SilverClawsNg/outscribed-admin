...
import { useAdminStore } from '../../gatekeeper/stores/AdminStore';
...
import { RoleTypeDescriptions } from '@/utils/descriptors'

// --- INITIALIZE STORES ---
const adminStore = useAdminStore();
const adminFilterStore = useAdminListFilterStore();
...
async function initPage() {
  ...
  const { isClean } = adminFilterStore.rehydrate(route.query);
  ...
  if (!isClean) {
    ...
    await router.replace({
      path: route.path,
      query: adminFilterStore.getAsDictionary()
    })
  ...
  const cleanApiPath = adminFilterStore.buildApiPath(adminStore.baseRoute);

  // 3. Fetch from store
  const { success, error } = await adminStore.loadAdmins(cleanApiPath)
...
const openModal = (authenticationId: string) => {
  adminStore.viewedRows.add(authenticationId);
  modalStore.push('AdminDetail', 'Admin Details', authenticationId)
};

const handleKeyPress = (event: KeyboardEvent, item: any) => {
  if (event.key === 'Enter' || event.key === ' ') {
    event.preventDefault();
    openModal(item.authenticationId);
  }
};


const reset = async () => {
  // 1. Wipe out any loaded admin arrays or pagination tokens from your main store
   adminStore.reset();

  // 2. Instruct the filter store to reset its internal states to defaults
  adminFilterStore.reset();
  ...
  <template v-else>
    ...
    <div class="shared__page-title">
      ...
      <InfiniteScroller
        ...
        :error="adminStore.loadMoreError"
        @load-more="adminStore.loadMoreAdmins"
        @retry="adminStore.loadMoreAdmins">

      <DisplayComponent
        :items="adminStore.admins"
        :item-key="(item) => item.authenticationId"
        :is-row-highlighted="(item) => adminStore.viewedRows.has(item.authenticationId)"
        @row-click="(item: any) => openModal(item.authenticationId)"
        ...
        <template #row="{ item }">
          <td>{{ toLongDate(item.assignedAt) }}</td>
           <td>{{ RoleTypeDescriptions[item.role] }}</td>
            <td>{{ item.username }}</td>
           <td>{{ item.isActive }}</td>
        </template>
      </DisplayComponent>
