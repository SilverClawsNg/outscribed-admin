...
onBeforeMount(() => {
 ...
 // --- INITIALIZE FORM DATA FROM STORE ---
  ...
  formData.value.type = adminStore.admin.role
  resetProgress()
})
...
// 1. Tell the ecosystem the admin has initiated an action
 ...
 const { success, error } = await adminStore.deactivateRole(formData.value!)
...
<template>
    ...
    <h2>Confirm to deactivate role</h2>
   ...
   <div class="button-holder">
          ...
          >
            {{ progressState.type === 'Loading' ? 'Submitting...' : 'Deactivate' }}
          </button>
        </div>
    </form>
  </div>
