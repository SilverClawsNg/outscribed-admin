...
import type { ResetRequest } from '../types/GatekeeperTypes'
...
const formData = ref<ResetRequest>({
   authenticationId: '',
  confirm: false
})
...
// 1. Tell the ecosystem the admin has initiated an action
 ...
 const { success, error } = await adminStore.resetSecurityStatus(formData.value!)
...
<template>
    ...
    <h2>Confirm to reset security status</h2>
  ...
  <FormProgress :progress="progressState" />
   ...
   <div class="button-holder">
          ...
          >
            {{ progressState.type === 'Loading' ? 'Submitting...' : 'Reset' }}
          </button>
        </div>
    </form>
  </div>
