...
import { ref } from 'vue';
import { getValidGeneralSortType, getValidCountry, getValidDateScope } from '@/utils/validators'; 

export const useAdminListFilterStore = defineStore('adminListFilter', () => {
...
const authenticationid = ref<string | null>(null)
...
if (queryParameters && Object.keys(queryParameters).length > 0) {
  ...
  if(queryParameters.authenticationid){
    authenticationid.value = parseValue(queryParameters.authenticationid)
  }
  ...
  if(queryParameters.role){
    const validatedRoleType = getValidDateScope(queryParameters.role)
    scope.value = validatedRoleType || null
    if(!validatedRoleType) wasClean = false
  }
...
function getAsDictionary(): Record<string, string> {
  ...
  function buildApiPath(baseRoute: string, overridePointer?: string | number, anchor?: string | null): string {
    ...
    if (sort.value && sort.value !== '-1') 
     ...
     if (role.value && role.value !== '-1') 
      urlParams.append('role', role.value);
     ...
     if (authenticationid.value && authenticationid.value.trim() !== '') 
      urlParams.append('authenticationid', authenticationid.value);
  ...
  return {
    sort, startdate, enddate, scope, keyword, pointer, username, bypasscache, country, role,
