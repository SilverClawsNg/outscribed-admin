...
import type {LogListDto, GetLogListResponse, LogDetailDto} from '../types/LogTypes'
import { useLogListFilterStore } from './LogListFilterStore'
...
export const useLogStore = defineStore('logStore', () => {
   ...
   // State
  const logs = ref<LogListDto[]>([]);
  const log = ref<LogDetailDto | null>(null);

  //Filter
  const filterStore = useLogListFilterStore()
  ...
  const baseRoute = ref<string>('/api/logs/checklist'); 
  ...
  // 1. Initial Load Path
  async function loadLogs(apiPathWithFilters: string): Promise<{ success: boolean; error: APIError | null }> {
    ...
    try {
      ...
      const outcome = await getAsync<GetLogListResponse>(apiPathWithFilters, true, {} as GetLogListResponse,
      ...
      if (outcome.isSuccess && outcome.value?.logs?.length) {
  ...
  async function loadMoreLogs() {
    ...
    try {
        ...
        const outcome = await getAsync<GetLogListResponse>(nextPageUrl, true, {} as GetLogListResponse,
          feedController.signal
        )
    ...
    // Consideration 2: Reconcile updates if data was retrieved
         if (outcome.isSuccess && outcome.value?.logs?.length) {
          ...
          const existingIds = new HashSetOrSet(logs.value.map(t => t.logId));
          // 1. Filter out duplicates and immediately shape the raw inputs into valid DTO structures
          const freshItems = outcome.value.logs
            .filter((t: any) => !existingIds.has(t.logId));

           logs.value.push(...freshItems);
      ...
      // 1. Initial Load Path
    async function loadLog(logId: string): Promise<{ success: boolean; error: APIError | null }> {
      ...
      try {
        ...
        // Note: Assuming getAsync is part of your API client layer
        const outcome = await getAsync<LogDetailDto>(`api/logs/checklist/${logId}`, true, {} as LogDetailDto, feedController.signal);
        ...
        if (!outcome.value) {
          ...
          const error = new APIError(
            404,
            'Not Found!',
            'Sorry. Sorry. We could not find the log you requested. It may have been removed, hidden, or archived.'
          )
          
              return { success: false, error: error }
        }
    
       log.value = outcome.value;
 ...
 function reset() {
  ...
  function abort() {
   ...
   return {
    logs, log, isFetchingMore, loadMoreError,hasNext, pointer, baseRoute, viewedRows,
    setBaseRoute, loadLogs, loadMoreLogs, loadLog, reset, abort
  };
