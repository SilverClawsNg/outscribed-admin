...
import { ref } from 'vue';
import { getValidGeneralSortType, getValidDateScope, getValidFlagType, getValidLimitedContentType } from '@/utils/validators'; 

export const useFlagListFilterStore = defineStore('flagListFilter', () => {
...
if (queryParameters && Object.keys(queryParameters).length > 0) {
  ...
  if(queryParameters.flaggerid){
    flaggerid.value = parseValue(queryParameters.flaggerid)
  }
  ...
  if(queryParameters.flagtype){
    ...
    flagtype.value = validatedType || '-1'
    ...
    if (sort.value && sort.value !== '-1') 
     ...
     if (startdate.value && startdate.value.trim() !== '') 
      ...
      urlParams.append('contentid', contentid.value);
          ...
          if (flaggerid.value && flaggerid.value.trim() !== '') 
      urlParams.append('flaggerid', flaggerid.value);
