...
import { ref } from 'vue';
import { getValidGeneralSortType, getValidDateScope, getValidContactType, getValidLimitedContentType } from '@/utils/validators'; 

export const useShareListFilterStore = defineStore('shareListFilter', () => {
...
if (queryParameters && Object.keys(queryParameters).length > 0) {
  ...
  if(queryParameters.sharerid){
    sharerid.value = parseValue(queryParameters.sharerid)
  }
 ...
 if(queryParameters.contenttype){
  ...
  if(queryParameters.contact){
    ...
    contact.value = validatedContact || '-1'
    if(!validatedContact) wasClean = false
  }
...
function getAsDictionary(): Record<string, string> {
    ...
    if (sort.value && sort.value !== '-1') 
     ...
     if (startdate.value && startdate.value.trim() !== '') 
      ...
      urlParams.append('contentid', contentid.value);
          ...
          if (sharerid.value && sharerid.value.trim() !== '') 
      urlParams.append('sharerid', sharerid.value);
  ...
  return {
    sort, startdate, enddate, scope, pointer, username, bypasscache, contenttype, sharerid, contentid, contact,
