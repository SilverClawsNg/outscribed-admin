...
onBeforeMount(() => {
  ...
  if (!commentStore.comment) {
    ...
    setError({
      ...
      detail: "Unable to load current comment details. Refresh page and try again",
 ...
 // --- INITIALIZE FORM DATA FROM STORE ---
  formData.value.commentId = commentStore.comment.commentId
  resetProgress()
...
// 1. Tell the ecosystem the user has initiated an action
 ...
 const { success, error } = await commentStore.certifyComment(formData.value!)
...
<template>
    ...
    <h2>Confirm to certify comment</h2>
   ...
   <div class="button-holder">
          ...
          >
            {{ progressState.type === 'Loading' ? 'Submitting...' : 'Certify' }}
          </button>
        </div>
    </form>
  </div>
