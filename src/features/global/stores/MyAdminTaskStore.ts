...
import type {AdminTaskListDto, GetAdminTaskListResponse} from '../types/GlobalTypes'
import { useAdminTaskListFilterStore } from './AdminTaskListFilterStore'
...
export const useMyAdminTaskStore = defineStore('myAdminTaskStore', () => {

   // State
  const tasks = ref<AdminTaskListDto[]>([]);

  //Filter
  const filterStore = useAdminTaskListFilterStore()
  ...
  const baseRoute = ref<string>('/api/tasks/checklist/my'); 
  ...
  // 1. Initial Load Path
  async function loadTasks(apiPathWithFilters: string): Promise<{ success: boolean; error: APIError | null }> {
    ...
    try {
      ...
      const outcome = await getAsync<GetAdminTaskListResponse>(apiPathWithFilters, true, {} as GetAdminTaskListResponse,
      ...
      if (outcome.isSuccess && outcome.value?.tasks?.length) {
        ...
        tasks.value = outcome.value.tasks;
  ...
  async function loadMoreTasks() {
    ...
    try {
        ...
        const outcome = await getAsync<GetAdminTaskListResponse>(nextPageUrl, true, {} as GetAdminTaskListResponse,
          feedController.signal
        )
    ...
    // Consideration 2: Reconcile updates if data was retrieved
         if (outcome.isSuccess && outcome.value?.tasks?.length) {
          ...
          const existingIds = new HashSetOrSet(tasks.value.map(t => t.adminTaskId));
          // 1. Filter out duplicates and immediately shape the raw inputs into valid DTO structures
          const freshItems = outcome.value.tasks
            .filter((t: any) => !existingIds.has(t.adminTaskId));

           tasks.value.push(...freshItems);
 ...
 function reset() {
  ...
  function abort() {
   ...
   return {
    tasks, isFetchingMore, loadMoreError,hasNext, pointer, baseRoute, viewedRows,
    setBaseRoute, loadTasks, loadMoreTasks, reset, abort
  };
