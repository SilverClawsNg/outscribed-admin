...
import type {TagListDto, GetTagListResponse, TagDetailDto} from '../types/TagTypes'
import { useTagListFilterStore } from './TagListFilterStore'
...
export const useTagStore = defineStore('tagStore', () => {
  ...
  const tag = ref<TagDetailDto | null>(null);

  //Filter
  const filterStore = useTagListFilterStore()
  ...
  const baseRoute = ref<string>('/api/tags/checklist'); 
  ...
  // 1. Initial Load Path
  async function loadTags(apiPathWithFilters: string): Promise<{ success: boolean; error: APIError | null }> {
    ...
    try {
      ...
      const outcome = await getAsync<GetTagListResponse>(apiPathWithFilters, true, {} as GetTagListResponse,
      ...
      if (outcome.isSuccess && outcome.value?.tags?.length) {
  ...
  async function loadMoreTags() {
    ...
    try {
        ...
        const outcome = await getAsync<GetTagListResponse>(nextPageUrl, true, {} as GetTagListResponse,
          feedController.signal
        )
          ...
          if (outcome.error) {
            loadMoreError.value = outcome.error
                ...
                loadMoreError.value = new APIError(
                    500,
                    'Unknown Error!',
                    'Unknown error occured while retrieving tag. Refresh page and try again.'
                );
            }
    ...
    // Consideration 2: Reconcile updates if data was retrieved
         if (outcome.isSuccess && outcome.value?.tags?.length) {
          ...
          const freshItems = outcome.value.tags
            .filter((t: any) => !existingIds.has(t.tagId));
    ...
    // 1. Initial Load Path
  async function loadTag(tagId: string): Promise<{ success: boolean; error: APIError | null }> {
    ...
    try {
      ...
      // Note: Assuming getAsync is part of your API client layer
      const outcome = await getAsync<TagDetailDto>(`api/tags/checklist/${tagId}`, true, {} as TagDetailDto, feedController.signal);
 ...
 function reset() {
  ...
  function setBaseRoute(apiUrl: any) {
    // We clone it using spread operator so the tag doesn't alter 
  ...
  function abort() {
   ...
   return {
    tags, tag, isFetchingMore, loadMoreError,hasNext, pointer, baseRoute, viewedRows,
    setBaseRoute, loadTags, loadMoreTags, loadTag, reset, abort
  };
