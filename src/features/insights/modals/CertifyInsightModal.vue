...
import type { ConfirmRequest } from '../types/InsightTypes'
...
onBeforeMount(() => {
 ...
 // --- INITIALIZE FORM DATA FROM STORE ---
  formData.value.insightId = insightStore.insight.insightId
  resetProgress()
...
// 1. Tell the ecosystem the user has initiated an action
 ...
 const { success, error } = await insightStore.certifyInsight(formData.value!)
...
<template>
    ...
    <h2>Confirm to certify insight</h2>
