...
import { InsightStatusDescriptions } from '@/utils/descriptors'
...
const openModal = (insightId: string) => {
  insightStore.viewedRows.add(insightId);
  modalStore.push('InsightDetail', 'Insight Details', insightId)
};

const handleKeyPress = (event: KeyboardEvent, item: any) => {
  if (event.key === 'Enter' || event.key === ' ') {
    event.preventDefault();
    openModal(item.insightId);
  }
};


const reset = async () => {
  // 1. Wipe out any loaded insight arrays or pagination tokens from your main store
   insightStore.reset();

  // 2. Instruct the filter store to reset its internal states to defaults
  insightFilterStore.reset();
 ...
 <template v-else-if="loadingError">
  ...
  <template v-else>
    ...
    <div class="shared__page-title">
      ...
      <DisplayComponent
        :items="insightStore.insights"
        :item-key="(item) => item.insightId"
        :is-row-highlighted="(item) => insightStore.viewedRows.has(item.insightId)"
        @row-click="(item: any) => openModal(item.insightId)"
        ...
        <template #row="{ item }">
          ...
          <td>{{ InsightStatusDescriptions[item.status] }}</td>
