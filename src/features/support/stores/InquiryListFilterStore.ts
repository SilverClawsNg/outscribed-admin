...
import { ref } from 'vue';
import { getValidGeneralSortType, getValidDateScope, getValidInquiryStatus
} from '@/utils/validators'; 

export const useInquiryListFilterStore = defineStore('inquiryListFilter', () => {
...
if (queryParameters && Object.keys(queryParameters).length > 0) {
  ...
  if(queryParameters.status){
    const validatedStatus = getValidInquiryStatus(queryParameters.status)
