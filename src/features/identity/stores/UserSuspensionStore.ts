...
import type {UserSuspensionListDto, GetUserSuspensionListResponse, UserSuspensionDetailDto, AssignRoleRequest, 
  ConfirmRequest} from '../types/IdentityTypes'
import { useUserSuspensionListFilterStore } from './UserSuspensionListFilterStore'
...
export const useUserSuspensionStore = defineStore('userSuspensionStore', () => {

   // State
  const suspensions = ref<UserSuspensionListDto[]>([]);
  ...
  const baseRoute = ref<string>('/api/users/suspensionlist'); 
  ...
  // 1. Initial Load Path
  async function loadUserSuspensions(apiPathWithFilters: string): Promise<{ success: boolean; error: APIError | null }> {
    ...
    try {
      ...
      const outcome = await getAsync<GetUserSuspensionListResponse>(apiPathWithFilters, true, {} as GetUserSuspensionListResponse,
      ...
      if (outcome.isSuccess && outcome.value?.suspensions?.length) {
        ...
        suspensions.value = outcome.value.suspensions;
  ...
  async function loadMoreUserSuspensions() {
    ...
    try {
        ...
        const outcome = await getAsync<GetUserSuspensionListResponse>(nextPageUrl, true, {} as GetUserSuspensionListResponse,
          feedController.signal
        )
    ...
    // Consideration 2: Reconcile updates if data was retrieved
         if (outcome.isSuccess && outcome.value?.suspensions?.length) {
          ...
          const existingIds = new HashSetOrSet(suspensions.value.map(t => t.id));
          // 1. Filter out duplicates and immediately shape the raw inputs into valid DTO structures
          const freshItems = outcome.value.suspensions
            .filter((t: any) => !existingIds.has(t.id));

           suspensions.value.push(...freshItems);
 ...
 function reset() {
  ...
  function abort() {
   ...
   return {
    suspensions, isFetchingMore, loadMoreError,hasNext, pointer, baseRoute, viewedRows,
    setBaseRoute, loadUserSuspensions, loadMoreUserSuspensions, reset, abort
  };
