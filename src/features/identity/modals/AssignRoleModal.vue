...
import type { AssignRoleRequest } from '../types/IdentityTypes'
...
const formData = ref<AssignRoleRequest>({
   accountId: '',
   type: '-1',
   confirm: false
})
...
// 1. Tell the ecosystem the user has initiated an action
 ...
 const { success, error } = await userStore.assignRole(formData.value!)
...
<template>
   ...
   <div class="button-holder">
          ...
          >
            {{ progressState.type === 'Loading' ? 'Submitting...' : 'Assign' }}
          </button>
        </div>
    </form>
  </div>
