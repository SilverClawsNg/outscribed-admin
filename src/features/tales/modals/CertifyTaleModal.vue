...
import type { ConfirmRequest } from '../types/TaleTypes'
...
onBeforeMount(() => {
 ...
 // --- INITIALIZE FORM DATA FROM STORE ---
  formData.value.taleId = taleStore.tale.taleId
  resetProgress()
...
// 1. Tell the ecosystem the user has initiated an action
 ...
 const { success, error } = await taleStore.certifyTale(formData.value!)
